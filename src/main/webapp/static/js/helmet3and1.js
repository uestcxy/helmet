...
function initAllClientMarkers() {
    ...
    $.each(list, function (idx, serverClient) {
        ...
        helmetClient.neUsername = serverClient.two;
...
function showHelmetInMap(helmetClient) {
    ...
    var labelContent = (helmetClient.online ? "在线" : "离线") + " " + helmetClient.neUserName + " " + gps.createTimeString;
...
function createCollectionItem(helmetClient){
    ...
    var online = helmetClient.online;
    var gps = helmetClient.gps;
    var li = $("<li data-gps='" + (gps.lon + ',' + gps.lat + ',' + gps.createTimeString) + "' data-clientId='" + clientId + "' data-name='" + neUserName + "' data-online='" + online + "'>" +
        // "<i alt='Avatar' class='img-circle pull-left avatar fa fa-helmet'></i>" +
        "<p><a onclick='viewHelmet(this)' style='cursor:pointer' title='点击查看设备详情'>" + neUserName + "</a> "+
...
function createCurrentHelmetWebsocket(){
    ...
    ajaxGet("/device/gettoken?clientId="+clientId,function(resp){
       if(resp.success){
           var token = resp.data;
           var oneHelmetWebSocketQueryString = "?token="+token+"&dataType=gps_sensor_heart";
           ...
           helmet3And1Page.one_websocket = new HelmetWebSocket(clientId,function(dataText){
               ...
               if(dataObj.dataType == 'gps'){
                   ...
                   updateWatchChartData(dataObj.battery,dataObj.concert,dataObj.relax);
...
function doSearchHelmet(){
    ...
    $.each(helmet3And1Page.clientList, function (idx, serverClient) {
        var clientId = serverClient.one;
        if(clientId == helmetSearchKeyword){
            found = true;
            var helmetClient = {};
            helmetClient.clientId = serverClient.one;
            helmetClient.neUsername = serverClient.two;

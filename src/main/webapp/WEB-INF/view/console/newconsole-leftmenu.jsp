        ...
        $.post("/rolemenu/menulist",{id:$("#userId").val(),isUserId:true},function(resp){
            if(resp.success){
                ...
                $.each(resp.data.menu1,function(idx,m1){
                    ...
                    var html = '<li><a href="#' + strmenu + '" data-toggle="collapse" class="collapsed"><i class="lnr lnr-file-empty"></i><span>' + m1.name + '</span><i class="icon-submenu lnr lnr-chevron-left"></i></a>'
                        + '<div id="' + strmenu + '" class="collapse ">'
                        + '<ul class="nav">';
                    $.each(resp.data.menu2,function(idx,m2){
                        if(m2.url != null){
                            if(m2.fatherId == m1.id){
                                html += '<li><a data-href="' + m2.url + '">' + m2.name + '</a></li>';
                            }
                        } else {

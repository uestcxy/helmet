...
import com.tianyi.helmet.server.dao.file.VideoDao;
...
//                    if(vclInfo != null) vclName = vclInfo.toString();
    ...
    public ResponseVo addNewFile(byte[] bytes, String origName, String suffix, String description, Date createTime,
                                 int userId, String imei, String neUserName, String src, String machineCode, String tag, String orderNo, Float lon, Float lat) {
        logger.debug("有新的视频文件要处理.");
        return addNewFile(bytes, origName, suffix, description, Dates.toLocalDateTime(createTime), userId, imei, neUserName, src, machineCode, tag, orderNo, lon, lat);
    }

...
//        logger.debug("收到mqtt消息.topic="+topicName+",内容"+json);
    ...
    public void processHelmetSensorData(String topicName, MqttMessage msg) {
        ...
        Integer concert = jo.getInteger("concert");
        Integer relax = jo.getInteger("relax");
        ...
        sensor.setClientId(imei);
        sensor.setCreateTime(createTime);
        if (concert != null)
            sensor.setConcert(concert);
        if (relax != null)
            sensor.setRelax(relax);
        sensor.setXa(xa);
        sensor.setYa(ya);
        sensor.setZa(za);
        sensor.setXg(xg);
        ...
        if (!StringUtils.isEmpty(battery)) {
            if (battery.endsWith("%")) {
                battery = battery.substring(0, battery.length());
            }
            sensor.setBattery(Integer.parseInt(battery.trim()));
        }
        ...
        if (!StringUtils.isEmpty(runtime)) {
            if (runtime.contains(":")) {
                ...
                if (runtimeLong.longValue() > Integer.MAX_VALUE) {
                    logger.debug("mqtt收到头盔设备运行时间是个Long值:" + runtimeLong + ",imei=" + imei);
                } else {
...
//        log.setLogContent(content);
    ...
    public void processNewStateData(String topicName, MqttMessage msg) {
        ...
        helmetHeartBeatService.insert(helmethb);
    ...
    * */
    public void processMindWaverData(String topicName, MqttMessage msg) {
        ...
        helmetMindWaverService.insert(helmetmv);
    ...
    * 2018-11-7 xiayuan
    * */
    public void processDeviceData(String topicName, MqttMessage msg) {
        JSONObject jo = parseMqttMessage(topicName, msg);
        if (jo == null)
            return;

        HelmetDeviceInfo helmetdi = new HelmetDeviceInfo();
        helmetdi.setClientId(jo.getString("imei"));
        helmetdi.setCategoryId(Integer.parseInt(jo.getString("categoryId")));
        helmetdi.setVersion(jo.getString("version"));

        noticeToClient(helmetdi);
    }

    public void noticeToClient(HelmetData helmetData) {
        logger.debug("数据通知websocket:" + helmetData.getClientId() + ",cls=" + helmetData.getClass().getSimpleName());
